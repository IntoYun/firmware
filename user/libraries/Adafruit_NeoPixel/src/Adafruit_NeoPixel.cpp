...
void Adafruit_NeoPixel::show(void) 
{
    ...
    *ptr = pixels,   // Pointer to next byte
    ...
    if(type == WS2812B) 
    { // same as WS2812, 800 KHz bitstream
        while(i)
        { // While bytes left... (3 bytes = 1 pixel)
            ...
            do 
            {
               ...
               // PIN_MAP[pin].gpio_peripheral->BSRR = PIN_MAP[pin].gpio_pin; // HIGH
                ...
                { // if masked bit is high
                    ...
                    pinResetFast(pin);//PIN_MAP[pin].gpio_peripheral->BRR = PIN_MAP[pin].gpio_pin; // LOW
                    asm volatile(
                    "mov r0, r0" "\n\t" "nop" "\n\t" "nop" "\n\t" "nop" "\n\t" "nop" "\n\t"
                    "\n\t" "nop" "\n\t" "nop" "\n\t" "nop" "\n\t"
                    ::: "r0", "cc", "memory");
    ...
    { // WS2811, 400 KHz bitstream
        ...
        { // While bytes left... (3 bytes = 1 pixel)
            ...
            do 
            {
                pinSetFast(pin);//PIN_MAP[pin].gpio_peripheral->BSRR = PIN_MAP[pin].gpio_pin; // HIGH
                if (c & mask) 
                { // if masked bit is high
    ...
    { // TM1803 (Radio Shack Tri-Color Strip), 400 KHz bitstream
        while(i) 
        { // While bytes left... (3 bytes = 1 pixel)
            mask = 0x800000; // reset the mask
            i = i-3;      // decrement bytes remaining
    ...
    { // must be only other option TM1829, 800 KHz bitstream
        while(i) 
        { // While bytes left... (3 bytes = 1 pixel)
            mask = 0x800000; // reset the mask
            i = i-3;      // decrement bytes remaining
