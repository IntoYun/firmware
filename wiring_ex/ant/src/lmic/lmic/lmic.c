...
static bit_t decodeFrame (void) {
    ...
    if( dlen < OFF_DAT_OPTS+4 ||
      ...
      norx:
        ...
        DEBUG("%lu: Invalid downlink, window=%s", os_getTime(), window);
#endif
        LMIC.dataLen = 0;
        return 0;
    }
...
#if LMIC_DEBUG_LEVEL > 0
    DEBUG("%lu: Received downlink, window=%s, port=%d, ack=%d", os_getTime(), window, port, ackup);
#endif
    return 1;
}
    ...
    if( dlen > LEN_JA ) {
        ...
        for( u1_t chidx=3; chidx<8; chidx++, dlen+=3 ) {
            ...
            if( freq ) {
                ...
                DEBUG("%lu: Setup channel, idx=%d, freq=%lu", os_getTime(), chidx, (unsigned long)freq);
#endif
            }
        }
    }
...
static void engineUpdate (void) {
#if LMIC_DEBUG_LEVEL > 0
    DEBUG("%lu: engineUpdate, opmode=0x%x", os_getTime(), LMIC.opmode);
#endif
