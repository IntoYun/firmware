...
/*debug switch*/
#define SYSTEM_MQTTCLIENT_DEBUG

#ifdef SYSTEM_MQTTCLIENT_DEBUG
#define SMQTTCLIENT_DEBUG(...)  do {DEBUG(__VA_ARGS__);}while(0)
#define SMQTTCLIENT_DEBUG_D(...)  do {DEBUG_D(__VA_ARGS__);}while(0)
#else
#define SMQTTCLIENT_DEBUG(...)
#define SMQTTCLIENT_DEBUG_D(...)
#endif
...
MqttClientClass::MqttClientClass(IPAddress addr, uint16_t port, Client& client) {
    this->_state = MQTT_DISCONNECTED;
    setServer(addr, port);
    setClient(client);
    this->stream = NULL;
...
boolean MqttClientClass::connect(const char *id, const char *user, const char *pass) {
    return connect(id,user,pass,0,0,0,0);
}

boolean MqttClientClass::connect(const char *id, const char* willTopic, uint8_t willQos, boolean willRetain, const char* willMessage) {
    return connect(id,NULL,NULL,willTopic,willQos,willRetain,willMessage);
}

boolean MqttClientClass::connect(const char *id, const char *user, const char *pass, const char* willTopic, uint8_t willQos, boolean willRetain, const char* willMessage) {
    if (!connected()) {
        int result = 0;
            ...
            if (len > 0) {
                ...
                if (type == MQTTPUBLISH) {
                    if (callback) {
                        ...
                        memcpy(buffer+llen+2,buffer+llen+3,tl); /* move topic inside buffer 1 byte to front */
...
boolean MqttClientClass::publish(const char* topic, const char* payload, boolean retained) {
    return publish(topic,(const uint8_t*)payload,strlen(payload),retained);
}

boolean MqttClientClass::publish(const char* topic, const uint8_t* payload, unsigned int plength) {
    return publish(topic, payload, plength, false);
}

boolean MqttClientClass::publish(const char* topic, const uint8_t* payload, unsigned int plength, boolean retained) {
    if (connected()) {
...
boolean MqttClientClass::subscribe(const char* topic, uint8_t qos) {
    if (qos < 0 || qos > 1) {
        return false;
    }
...
boolean MqttClientClass::unsubscribe(const char* topic) {
    if (MQTT_MAX_PACKET_SIZE < 9 + strlen(topic)) {
        // Too long
        return false;
    }
...
MqttClientClass& MqttClientClass::setServer(uint8_t * ip, uint16_t port) {
    IPAddress addr(ip[0],ip[1],ip[2],ip[3]);
    return setServer(addr,port);
}

MqttClientClass& MqttClientClass::setServer(IPAddress ip, uint16_t port) {
    this->ip = ip;
    this->port = port;
    this->domain = NULL;
    return *this;
...
MqttClientClass& MqttClientClass::setServer(const char * domain, uint16_t port) {
    this->domain = domain;
    this->port = port;
    return *this;
}

MqttClientClass& MqttClientClass::setCallback(MQTT_CALLBACK_SIGNATURE) {
    this->callback = callback;
    return *this;
}

MqttClientClass& MqttClientClass::setClient(Client& client){
    this->_client = &client;
    return *this;
}
