...
{
    aJsonObject* node = (aJsonObject*)pvPortMalloc(sizeof(aJsonObject));
    MO_ASSERT((node!=NULL));
    if (node)
...
{
    char* outBuf = (char*)pvPortMalloc(1024); /* XXX: Dynamic size. */
    MO_ASSERT((outBuf!=NULL));
    if (outBuf == NULL)
    {
        return NULL;
    }
...
void aJsonClass::addItemToObject(aJsonObject *object, const char *string, aJsonObject *item)
{
    ...
    //item->name = strdup(string);
		item->name=(char *)pvPortMalloc(strlen(string)+1);
...
void aJsonClass::replaceItemInObject(aJsonObject *object, const char *string, aJsonObject *newitem)
{
    ...
    if (c)
    {
	    ...
	    //newitem->name = strdup(string);
			newitem->name=(char *)pvPortMalloc(strlen(string)+1);
...
aJsonObject *aJsonClass::createItem(const char *string)
{
    ...
    if (item)
    {
        ...
        //item->valuestring = strdup(string);
				...
				item->valuestring=(char *)pvPortMalloc(strlen(string)+1);
