...
#define FLASH_SYSTEM_PARAMS_FLASH_END_ADDRESS          ((uint32_t)0x0801FFFF)
...
void read_boot_params(boot_params_t *pboot_params) {
    ...
    for (int num = 0; num<len; num++) {
        pboot[num] = EEPROM.HAL_EEPROM_Read(address+num);
    }
}
...
boot_params_t &HAL_Boot_Param(void) {
    return intorobot_boot_params;
}

system_params_t &HAL_System_Param(void) {
    return intorobot_system_params;
}
