...
static bool i2cCheckLineState(int8_t sda, int8_t scl){
    ...
    __pinMode(sda, ESP32_PULLUP|ESP32_OPEN_DRAIN|ESP32_INPUT);
    __pinMode(scl, ESP32_PULLUP|ESP32_OPEN_DRAIN|ESP32_OUTPUT);

    if(!__digitalRead(sda) || !__digitalRead(scl)) { // bus in busy state
    ...
    if(!__digitalRead(sda) || !__digitalRead(scl)) { // bus in busy state
        //log_e("Bus Invalid State, TwoWire() Can't init sda=%d, scl=%d",__digitalRead(sda),__digitalRead(scl));
...
i2c_err_t i2cAttachSCL(i2c_t * i2c, int8_t scl)
{
    ...
    __digitalWrite(scl, HIGH);
    __pinMode(scl, ESP32_OPEN_DRAIN | ESP32_PULLUP | ESP32_INPUT | ESP32_OUTPUT);
...
i2c_err_t i2cDetachSCL(i2c_t * i2c, int8_t scl)
{
    ...
    __pinMode(scl, ESP32_INPUT | ESP32_PULLUP);
    return I2C_ERROR_OK;
}
...
i2c_err_t i2cAttachSDA(i2c_t * i2c, int8_t sda)
{
    ...
    __digitalWrite(sda, HIGH);
    __pinMode(sda, ESP32_OPEN_DRAIN | ESP32_PULLUP | ESP32_INPUT | ESP32_OUTPUT );
...
i2c_err_t i2cDetachSDA(i2c_t * i2c, int8_t sda)
{
    ...
    __pinMode(sda, ESP32_INPUT | ESP32_PULLUP);
    return I2C_ERROR_OK;
}
