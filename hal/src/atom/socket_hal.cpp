...
#include "socket/openwrt_conn.h"

OpenwrtConnClass openwrt_conn;
...
int32_t socket_connect(sock_handle_t sd, const sockaddr_t *addr, long addrlen)
{
    ...
    bool result = openwrt_conn.socketConnect(sd, ip, port);
    return (result ? 0 : 1);
}
...
sock_result_t socket_receive(sock_handle_t sd, void* buffer, socklen_t len, system_tick_t _timeout)
{
    ...
    if (_timeout==0) {
        result = openwrt_conn.socketReadable(sd);
    ...
    if (!result)
        result = openwrt_conn.socketRecv(sd, (char*)buffer, len);
    return result;
}
...
sock_result_t socket_receivefrom(sock_handle_t sock, void* buffer, socklen_t bufLen, uint32_t flags, sockaddr_t* addr, socklen_t* addrsize)
{
    ...
    sock_result_t result = openwrt_conn.socketReadable(sock);
    if (result<=0)// error or no data
        return result;

    // have some data to let's get it.
    result = openwrt_conn.socketRecvFrom(sock, &ip, &port, (char*)buffer, bufLen);
    if (result > 0) {
        uint32_t ipv4 = ip;
...
{
    bool result = openwrt_conn.socketIsConnected(socket);
    return (result ? 0 : 1);
}

sock_result_t socket_close(sock_handle_t sock)
{
    bool result = openwrt_conn.socketFree(sock); // closes and frees the socket
    return (result ? 0 : 1);
}

sock_result_t socket_send(sock_handle_t sd, const void* buffer, socklen_t len)
{
    return openwrt_conn.socketSend(sd, (const char*)buffer, len);
}
