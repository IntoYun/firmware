...
{
    CHECK_SUCCESS(CellularMDM.powerOn());
    return 0;
}

cellular_result_t  cellular_init(void* reserved)
{
    CHECK_SUCCESS(CellularMDM.init());
    return 0;
}
...
{
    CHECK_SUCCESS(CellularMDM.registerNet());
    return 0;
}
...
cellular_result_t  cellular_pdp_activate(CellularCredentials* connect, void* reserved)
{
    if (strcmp(connect->apn,"") != 0) {
        CHECK_SUCCESS(CellularMDM.pdp(connect->apn));
    }
    else {
        CHECK_SUCCESS(CellularMDM.pdp());
    }
    return 0;
}
...
{
    CHECK_SUCCESS(CellularMDM.disconnect());
    return 0;
}
...
cellular_result_t  cellular_gprs_attach(CellularCredentials* connect, void* reserved)
{
    if (strcmp(connect->apn,"") != 0 || strcmp(connect->username,"") != 0 || strcmp(connect->password,"") != 0 ) {
        CHECK_SUCCESS(CellularMDM.join(connect->apn, connect->username, connect->password));
    }
    else {
        CHECK_SUCCESS(CellularMDM.join());
    }
    return 0;
}
...
{
    CHECK_SUCCESS(CellularMDM.detach());
...
{
    NetStatus status;
    CHECK_SUCCESS(CellularMDM.getSignalStrength(status));
...
cellular_result_t cellular_data_usage_set(CellularDataHal* data, void* reserved)
{
    ...
    bool rv = CellularMDM.getDataUsage(data_usage);
