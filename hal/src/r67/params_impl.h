...
typedef struct
{
    ...
    uint8_t  reserve2[32];         // 状态预留区，没添加一个状态，预留区大小减1
    ...
    char     reserve3[52];         // 预留区
    char     http_domain[52];      // http服务器域名
    int      http_port;            // http服务器端口

    uint8_t  reserve[491];         // 参数预留区 每添加一个参数，预留区大小减1
    uint8_t  end;
}hal_system_params_t;
