...
extern void __pinMode(uint8_t pin, uint8_t mode) {
    ...
    if(pin < 16){
        ...
        } else if(mode & ESP8266_FUNCTION_0){
            GPC(pin) = (GPC(pin) & (0xF << GPCI)); //SOURCE(GPIO) | DRIVER(NORMAL) | INT_TYPE(UNCHANGED) | WAKEUP_ENABLE(DISABLED)
            GPEC = (1 << pin); //Disable
            GPF(pin) = GPFFS((mode >> 4) & 0x07);
            if(pin == 13 && mode == ESP8266_FUNCTION_4) GPF(pin) |= (1 << GPFPU);//enable pullup on RX
        }  else if(mode == ESP8266_OUTPUT || mode == ESP8266_OUTPUT_OPEN_DRAIN){
            GPF(pin) = GPFFS(GPFFS_GPIO(pin));//Set mode to GPIO
            GPC(pin) = (GPC(pin) & (0xF << GPCI)); //SOURCE(GPIO) | DRIVER(NORMAL) | INT_TYPE(UNCHANGED) | WAKEUP_ENABLE(DISABLED)
            if(mode == ESP8266_OUTPUT_OPEN_DRAIN) GPC(pin) |= (1 << GPCD);
            GPES = (1 << pin); //Enable
        } else if(mode == ESP8266_INPUT || mode == ESP8266_INPUT_PULLUP){
            GPF(pin) = GPFFS(GPFFS_GPIO(pin));//Set mode to GPIO
            GPEC = (1 << pin); //Disable
            GPC(pin) = (GPC(pin) & (0xF << GPCI)) | (1 << GPCD); //SOURCE(GPIO) | DRIVER(OPEN_DRAIN) | INT_TYPE(UNCHANGED) | WAKEUP_ENABLE(DISABLED)
            if(mode == ESP8266_INPUT_PULLUP) {
                GPF(pin) |= (1 << GPFPU);  // Enable  Pullup
            }
        } else if(mode == ESP8266_WAKEUP_PULLUP || mode == ESP8266_WAKEUP_PULLDOWN){
            GPF(pin) = GPFFS(GPFFS_GPIO(pin));//Set mode to GPIO
            GPEC = (1 << pin); //Disable
            if(mode == ESP8266_WAKEUP_PULLUP) {
                GPF(pin) |= (1 << GPFPU);  // Enable  Pullup
        ...
        GPC16 = 0;
        if(mode == ESP8266_INPUT || mode == ESP8266_INPUT_PULLDOWN_16){
            if(mode == ESP8266_INPUT_PULLDOWN_16){
                GPF16 |= (1 << GP16FPD);//Enable Pulldown
            }
            GP16E &= ~1;
        } else if(mode == ESP8266_OUTPUT){
            GP16E |= 1;
        }
    }
}
